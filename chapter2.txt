
from selenium import webdriver
from selenium.webdriver.common.by import By
import time

# Optional - Keep the browser open
chrome_options = webdriver.ChromeOptions()
chrome_options.add_experimental_option("detach", True)

driver = webdriver.Chrome(options=chrome_options)

driver.get("http://orteil.dashnet.org/experiments/cookie/")

# Get the cookie element
cookie = driver.find_element(by=By.ID, value="cookie")

# Get upgrade item ids.
upgrades = driver.find_elements(by=By.CSS_SELECTOR, value="#store div")
upgrades_ids = [item.get_attribute("id") for item in upgrades]

# Set a timeout for the script (5 seconds)
timeout = time.time() + 5
five_min = time.time() + 60*5  # 5 minutes


# Loop that runs indefinitely
while True:
    # Click the cookie to generate more cookies
    cookie.click()

    # Every 5 seconds, check for available upgrades
    if time.time() > timeout:

        # Extract upgrade prices and convert them to integers
        all_prices = driver.find_elements(by=By.CSS_SELECTOR, value="#store b")
        upgrade_prices = []

        # Convert <b> text into an integer price.
        for price in all_prices:
            element_text = price.text
            if element_text != "":  # Check if not empty
                # Split by '-' sign, use second[1] element, strip() to remove unnecessary spaces, replace unnecessary ',' in 2,000
                cost = int(element_text.split("-")[1].strip().replace(",", ""))
                upgrade_prices.append(cost)

        # Create a dictionary of store items and prices
        cookie_upgrades = dict(zip(upgrade_prices, upgrades_ids))

        # Get the current cookie count
        money_element = driver.find_element(by=By.ID, value="money").text
        if "," in money_element:
            money_element = money_element.replace(",", "")
        cookie_count = int(money_element)

        # Find upgrades that can currently be afforded
        affordable_upgrades = {}
        for cost, id in cookie_upgrades.items():
            if cookie_count > cost:
                affordable_upgrades[cost] = id
        # Purchase the most expensive affordable upgrade
        if affordable_upgrades:
            highest_price_affordable_upgrade = max(affordable_upgrades)
            print(highest_price_affordable_upgrade)
            to_purchase_id = affordable_upgrades[highest_price_affordable_upgrade]

            # Click on the upgrade to purchase
            driver.find_element(by=By.ID, value=to_purchase_id).click()

        # Add another 5 seconds until the next check and reset the timeout
            
        timeout = time.time() + 5

    if time.time() > five_min:
            cookie_per_s = driver.find_element(by=By.ID, value="cps").text
            print(cookie_per_s)
            break
